...
export async function searchFlights(params: FlightSearchParams): Promise<FlightSearchResponse> {
  ...
  try {
    ...
    console.log(`🎫 Search request passenger counts:`, {
      original: { adults: params.adults, children: params.children, infants: params.infants },
      parsed: passengerCounts,
      total: passengerCounts.adults + passengerCounts.children + passengerCounts.infants
    });
    
    const flights = await transformEaseMyTripResponse(emtResponse, params.class, passengerCounts);
